# 🚀 Warp Orchestrator — TaskRouter + Router MCP (Auto-Loop, Logs, VSCode-Friendly)
...
├─ README.md
├─ instructions.md
├─ router_mcp.py # Router MCP server (with Markdown + JSONL persistence, auto-loop, workflow_id)
├─ orchestrator.sh # One-command launcher + live tail (macOS/Linux)
├─ orchestrator.ps1 # One-command launcher + live tail (Windows) [optional]
├─ warp_config/
   ├─ warp-agent-config.yaml
   ├─ warp-config.yaml
├─ docs/
│  ├─ CHANGELOG.md # Auto-appended final summaries per run
│  ├─ build-summary.md # Step-by-step log + final block
│  └─ router_log.jsonl # Structured event log
└─ project/
   ├─ docs/
   │  └─ site-spec.md # Source-of-truth spec for your website build
...
### 1) 🧑‍💻 Human prompt (TaskRouter chat)
Paste this (one message):
```
workflow_id: site-scaffold-001

Use /project/docs/site-spec.md as the source of truth. Execute the full build:
...
```
- Router ↔ TaskRouter iterate through FrontendDeveloper, etc., until GitWorkflow commits and TaskRouter returns `DONE`.

### 3) 🗃️ Logs & Final Output
...
- **Isolation**: Keep work under `project/`; specs in `/project/docs/site-spec.md`.

---

## 💡 Tips & Extensions
- **Spec-driven builds**: Put project rules in `/project/docs/site-spec.md` and reference it in prompts.
...
- **Kickoff via HIGHLEVEL**: Extend Router to accept `HIGHLEVEL: …` so you don’t craft the first routing line.
